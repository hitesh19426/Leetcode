class Solution {
    ...
    int helper(int ind, int last, vector<int>& prices){
        ...
        // if(last == -1)
        //     return dp[{ind, last}] = max(helper(ind+1, ind, prices), helper(ind+1, -1, prices));
        // return dp[{ind, last}] = max(arr[ind]-arr[last]+helper(ind+2, -1, prices), helper(ind+1, last, prices));
    }
public:
